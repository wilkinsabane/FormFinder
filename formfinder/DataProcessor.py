...
class DataProcessor:
    ...
    def process_league(self, league_id=None, fixtures=None, standings=None, teams=None, filepath=None):
        ...
        - filepath: Load data from CSV file (legacy mode)
        - league_id, fixtures, standings, teams: Process data directly (new mode)
        """
        if filepath is not None:
            ...
            # New mode: process data directly
            if not fixtures:
                logging.info(f"No fixtures provided for league {league_id}, cannot process league.")
                return pd.DataFrame(columns=['team_id', 'team_name', 'win_rate'])
            
            # Convert fixtures list to DataFrame
            matches_data = []
            for fixture in fixtures:
                if hasattr(fixture, '__dict__'):
                    # Convert Match object to dict
                    match_dict = fixture.__dict__
                else:
                    match_dict = fixture
                matches_data.append(match_dict)
            
            if not matches_data:
                logging.info(f"No valid match data for league {league_id}, cannot process league.")
                return pd.DataFrame(columns=['team_id', 'team_name', 'win_rate'])
            ...
            for col in required_columns:
                if col not in matches.columns:
                    logging.warning(f"Missing required column {col} in fixtures data")
                    return pd.DataFrame(columns=['team_id', 'team_name', 'win_rate'])
        else:
            logging.error("Either filepath or fixtures data must be provided")
            return pd.DataFrame(columns=['team_id', 'team_name', 'win_rate'])
